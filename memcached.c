...
static void drive_machine(conn *c) {
  ...
  while (!stop) {
    switch(c->state) {
    // 主要处理tcp连接,只有在主线程的下，才会执行listening监听操作
    case conn_listening:
      addrlen = sizeof(addr);
...
static bool _parse_slab_sizes(char *s, uint32_t *slab_sizes) {
  ...
  for (char *p = strtok_r(s, "-", &b);
    ...
    if (last_size >= size) {
      fprintf(stderr, "slab size %u cannot be lower than or equal to a "
        "previous class size\n", size);
      return false;
    }
    if (size <= last_size + CHUNK_ALIGN_BYTES) {
      fprintf(stderr, "slab size %u must be at least %d bytes larger "
