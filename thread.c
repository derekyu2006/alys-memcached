...
static void setup_thread(LIBEVENT_THREAD *me) {
  ...
  pthread_mutex_init(&me->stats.mutex, NULL) != 0;
...
static void thread_libevent_process(int fd, short which, void *arg) {
  ...
  switch (buf[0]) {
  case 'c':
    item = cq_pop(me->new_conn_queue); // 连接对列的中连接取一个.
    if (NULL != item) {
