...
// slab在分配内存的时候把具有相同大小的item划分为一类.
// heads和tails分别记录了每一类
static item *heads[LARGEST_ID];
static item *tails[LARGEST_ID];
static itemstats_t itemstats[LARGEST_ID]; // 每一个LRU队列有多少个item
...
void do_item_update(item *it) {
  MEMCACHED_ITEM_UPDATE(ITEM_key(it), it->nkey, it->nbytes);
  if (it->time < current_time - ITEM_UPDATE_INTERVAL) { // 如果最近才更新过, 就不要再更新了.
    assert((it->it_flags & ITEM_SLABBED) == 0);
...
// 根据cur_lru参数只遍历某一个区域(热、温、冷区)
// 主要触发LRU是在do_item_alloc方法分配创建对象的时候或者启动mc的时候
static int lru_pull_tail(const int orig_id, const int cur_lru,
    const uint64_t total_bytes, uint8_t flags) {
  item *it = NULL;
  int id = orig_id;
    ...
    switch (cur_lru) {
      ...
      case WARM_LRU:
        ...
        if (sizes_bytes[id] > limit) {
          ...
          do_item_update_nolock(search); // 刷新这个item的访问时间以及在LRU队列中的位置
          do_item_remove(search);
          item_trylock_unlock(hold_lock);
        } else {
