...
typedef struct _stritem {
  struct _stritem *next; // 记录LRU双向链表下一个item的地址
  struct _stritem *prev; // //记录LRU双向链表上一个item的地址
  struct _stritem *h_next; // 用于哈希表的冲突链
  rel_time_t    time;     // //最后一次访问时间,绝对时间
  rel_time_t    exptime; // 本item的过期时间.
  int       nbytes;   // 本item存放的数据的长度 + 2
  unsigned short  refcount; // 本item的引用数
  uint8_t     nsuffix;  // 后缀长度
  uint8_t     it_flags; // item的属性.
  uint8_t     slabs_clsid; // 本item所属的slab类型id. 具有相同的此id的item放在一起.
  uint8_t     nkey;  // 键值的长度
